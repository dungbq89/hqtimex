                    ...
                    <div class="pagination">
                        <ul>
                            <?php if ($pager->getPage() >= 2): ?>
                                <li><a href="<?php echo url_for1('@hq_news_search?q=' . htmlentities($queryName)) ?>">
                                        <i class="fa fa-angle-left"></i></a></li>
                            <?php endif; ?>
                            <?php foreach ($pager->getLinks() as $page): ?>
                                <li class="<?php echo $page == $pager->getPage() ? 'current' : '' ?>">
                                    <a href="<?php echo url_for1('@hq_news_search?page=' . $page . '&q=' . htmlentities($queryName)) ?>">
                                        <?php echo $page ?></a></li>
                            <?php endforeach; ?>
                            <?php if ($pager->getPage() < $pager->getLastPage()): ?>
                                <li>
                                    <a href="<?php echo url_for1('@hq_news_search?page=' . $pager->getLastPage() . '&q=' . htmlentities($queryName)) ?>"><i
